...
def plot_segments(material):
    ...
    for seg in segments:
        if len(seg.linear_k) > 2:
            ...
            xtick_labels.append(sorep.prettify.latex_kpoint_label(seg.stop_label))
            if seg.bands.shape[0] == 1:
                ax.plot(seg.linear_k, seg.bands[0], c="k")
            else:
                ax.plot(seg.linear_k, seg.bands[0], c="tab:blue")
                ax.plot(seg.linear_k, seg.bands[1], c="tab:red", linestyle="--")
    ...
    ax.set_title(f"PwBandsWorkChain<{material.metadata['calculation_uuid']}>", size=9)
    fig.suptitle(f'{sorep.prettify.unicode_chemical_formula(material.metadata["formula_hill"])}')
